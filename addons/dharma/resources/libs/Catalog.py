...
def get_lang():
    #lang = os.environ.get("LANG") # Ne fonctionne pas sur les plateformes non posix (None est retroune), de plus l'utilsateur peut vouloir choisir le langue lui meme => utilisation de settings
    #n = lang.split('_')
    #lang = "fr"
    #if n[0] in ("fr","de"):
    #    lang = n[0]
...
#Note idealement utiliser les mois provenant de strings.xml au lieu de les coder en dur, pour cela __language__ permet d'y avoir acces
...
class Catalog:
    ...
    def __init__(self):
        ...
        try:
            base_page_url = self.ARTE_WEB_ROOT + lang + "videos/" #we first load the page in order to get the page url wuth the correct index
