...
class Manager( xbmcgui.WindowXMLDialog ):
    ...
    def setIconFanartImages( self, idSet, listitem ):
        art = DATABASE.getArtForItem( idSet )
        if art:
            listitem.setIconImage( art[ "thumb" ] )
    ...
    def onContextMenu( self, controlID, listItem ):
        ...
        try:
            ...
            if selected == 0:
                ...
                if new_title is None: force = new_title
                force = force or new_title
                if force is not None and new_title != title:
