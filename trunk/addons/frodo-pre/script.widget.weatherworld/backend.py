...
class WeatherWorld:
    def __init__( self, pointer="radiobutton-focus.png" ):
    ...
    def getIconCache( self, url ):
        c_icon = os.path.join( weatherworld.ADDON_DATA, "icons", os.path.basename( url ) )
        OK = xbmcvfs.exists( c_icon )
        if not OK: OK = xbmcvfs.copy( url, c_icon )
        return ( url, c_icon )[ OK ]
