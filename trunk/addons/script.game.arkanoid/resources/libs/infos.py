...
def parse_description_xml():
    try:
        ...
        guid = elems.findtext( "guid" ) or elems.findtext( "id" )
        #(required) type:
        try:
            ...
            type = elems.findtext( "type" ).title()
        # (required) Title
        ...
        try: authors = [ elem.attrib for elem in elems.find( "authors" ) ]
        except: authors = elems.findtext( "author" )
        ...
        try: minrevision = " / ".join( [ elem.text for elem in elems.find( "minversion" ) ] )
        except: minrevision = ""
...
class Info( xbmcgui.WindowXMLDialog ):
    ...
    def onInit( self ):
        try:
            ...
            try: authors = " / ".join( [ "%s, %s" % ( author[ "name" ], author[ "email" ] ) for author in self.infos[ "authors" ] ] )
            except: authors = self.infos[ "authors" ]
