...
def parse_description_xml( addon ):
    try:
        # load and parse description.xml file
        feed = open( os.path.join( addon, "description.xml" ) )
        tree = ET.parse( feed )
        feed.close()
        elems = tree.getroot()
...
class Info( xbmcgui.WindowXMLDialog ):
    def __init__( self, *args, **kwargs ):
        ...
        self.addon_path = kwargs[ "addon" ]
        self.infos = parse_description_xml( self.addon_path )

    def onInit( self ):
        try:
            icon = os.path.join( self.addon_path, "default.tbn" )
            print icon
            self.getControl( 48 ).reset()
            listitem = xbmcgui.ListItem( self.infos[ "title" ], "", icon )
            ...
            #listitem.setProperty( "revision", __svn_revision__ )
            listitem.setProperty( "filepath", self.addon_path )
...
def show_info( addon=None ):
    dir_path = os.getcwd().replace( ";", "" )
    current_skin, force_fallback = getUserSkin()
    file_xml = "IPX-Infos.xml"

    w = Info( file_xml, dir_path, current_skin, force_fallback, addon=addon )
    w.doModal()
    del w
