...
class ItemDescriptionOld( xbmcgui.WindowXMLDialog ):
    # control id's
...
class ItemDescription( xbmcgui.WindowXMLDialog ):
    ...
    def _get_settings( self, defaults=False  ):
        """ reads settings from conf file """
        logger.LOG( logger.LOG_DEBUG, "_get_settings: ItemDescription")
        pass
...
class ItemDescriptionFTP( ItemDescription ):

    def __init__( self, *args, **kwargs ):
        #TODO: deplacer ce chemin dans le fichier de configuration
        self.serverDir  = "/.passionxbmc/Installeur-Passion/content/"
        
        # Appel __init__ super classe
        ItemDescription.__init__(self, *args, **kwargs )
...
class ItemDescriptionSQL( ItemDescription ):
    ...
    def _get_info( self, defaults=False  ):
        ...
        try:
            ...
            logger.LOG( logger.LOG_DEBUG, "ItemDescriptionSQL - _get_info: Item ID: %d", self.itemName )
            dico = browse.info(int(self.itemName)) # self.itemName dans ce cas est in id
            ...
            # Telechargement de l'image
            if previewPicture != None:
                self._downloadFile(previewPicture)
    ...
    def _downloadFile(self,remoteFilePath):
        """
        Fonction de telechargement commune : images ...
        """
        localFilePath = os.path.join(self.mainwin.CacheDir, os.path.basename(remoteFilePath))
        if os.path.exists( localFilePath ):
            ...
            try:
                logger.LOG( logger.LOG_DEBUG, "ItemDescriptionSQL - _get_info: _downloadFile: %s", localFilePath )
                #print("downloadJPG destination doesn't exist, try to retrieve")
                loc = urllib.URLopener()
                loc.retrieve( remoteFilePath, localFilePath )
            except:
                logger.LOG( logger.LOG_DEBUG, "_downloadFile: Exception - Impossible de telecharger le fichier: %s", remoteFilePath )
                logger.EXC_INFO( logger.LOG_ERROR, sys.exc_info(), self )
