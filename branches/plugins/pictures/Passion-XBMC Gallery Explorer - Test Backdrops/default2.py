...
def show_icones(cat):
    ...
    for id in dico['id']:
        ...
        if dico['type'][indice] == 'PIC':
            ...
            if setting_DL == "DL":
                url = '%s?download_path=%s' % ( sys.argv[ 0 ], repr( urllib.quote_plus( dico['image'][indice] ) ), )
            else:
...
if "cat" in parametres.keys():
    ...
    dl_path = urllib.unquote_plus( parametres["download_path"].replace('cat=',''))
    urllib.urlretrieve( dl_path, destination  )
...
fpath = "http://blablabla.jpg"
# si l'utilisateur veut le DL l'url est comme suis
if setting_DL == "DL":
    url = '%s?download_path=%s' % ( sys.argv[ 0 ], repr( urllib.quote_plus( fpath ) ), )
else:
    url = fpath
...
if ( "download_path=" in sys.argv[ 2 ] ):
    dl_path = urllib.unquote_plus( sys.argv[ 2 ].split( "=" )[ 1 ] )
    urllib.urlretrieve( dl_path, destination  )
    
download_path = xbmcplugin.getSetting( "download_path" )
if os.path.exists( download_path ):
    print "c'est bon on peut sauvegarder"
